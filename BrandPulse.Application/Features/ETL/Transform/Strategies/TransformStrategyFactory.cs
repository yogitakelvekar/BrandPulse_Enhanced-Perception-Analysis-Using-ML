...
namespace BrandPulse.Application.Features.ETL.Transform.Strategies
{
    public class TransformStrategyFactory : ITransformStrategyFactory
    {
        ...
        public IEnumerable<ITransformStrategy> GetStrategies(SocialMediaAggregates data)
        {
            ...
            if (data?.Tweets != null)
            {
                var tweetStrategy = new TweetTransformStrategy(serviceProvider.GetRequiredService<ISentimentDataTransform<Tweet>>(), 
            ...
            {
                var redditStrategy = new RedditTransformStrategy(serviceProvider.GetRequiredService<ISentimentDataTransform<RedditPost>>(), 
            ...
            {
                var youtubeStrategy = new YoutubeTransformStrategy(serviceProvider.GetRequiredService<ISentimentDataTransform<YouTubeVideo>>(), 
                    serviceProvider.GetRequiredService<IWordCloudDataTransform<YouTubeVideo>>(), serviceProvider.GetRequiredService<IInfluencerDataTransform<YouTubeVideo>>());
